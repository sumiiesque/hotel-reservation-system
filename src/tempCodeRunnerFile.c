    ...
    do {
        ...
        switch (choice) {
            ...
            case 8:
                ...
                makeReservation(res); break;
            case 9: {
                int count; room* avail = getAvailableRooms(&count);
                for (int i = 0; i < count; i++)
                    printf("%d | %s | Rs %.2f\n", avail[i].roomid, avail[i].type, avail[i].price);
                freeRoomList(avail); break;
            }
            case 10:
                printf("Enter Reservation ID to cancel: "); int resid; scanf("%d", &resid); getchar();
                if (cancel(resid)) printf("Reservation canceled.\n");
